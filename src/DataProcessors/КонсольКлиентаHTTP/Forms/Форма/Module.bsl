...
&НаКлиенте
Асинх Процедура ОтобразитьРазмерТелаОтвета()
	...
	Если ТелоОтветаВФайл Тогда
		...
		ФайлСуществует = Ждать Ф.СуществуетАсинх();
		Если ФайлСуществует Тогда
			РазмерОтвета = Ждать Ф.РазмерАсинх();
		КонецЕсли;
	Иначе
...
Асинх Процедура ПрочитатьФайлИмпорта(ИмяФайлаИмпорта)
	Отказ = Ложь;
	ОписаниеОшибки = "";
	НачалоТекстаСообщения = "Не удалось прочитать файл импорта по причине: ";
	
	ФайлИмпорта = Новый Файл(ИмяФайлаИмпорта);
	ФайлСуществует = Ждать ФайлИмпорта.СуществуетАсинх();
	Если НЕ ФайлСуществует Тогда
		СообщитьПользователю(НачалоТекстаСообщения + "отсутствует файл " + ИмяФайлаИмпорта);
		Возврат;
	КонецЕсли;
	
	ОбъектИмпорта = Ждать ОбъектФайлаИмпорта(ФайлИмпорта);
	Если ОбъектИмпорта.Отказ Тогда
		СообщитьПользователю(НачалоТекстаСообщения + ОбъектИмпорта.ОписаниеОшибки);
		Возврат;
	КонецЕсли;
	
	ВалидацияОбъектаИмпорта(ОбъектИмпорта.Данные, Отказ, ОписаниеОшибки);
	Если Отказ Тогда
		СообщитьПользователю(НачалоТекстаСообщения + ОписаниеОшибки);
		Возврат;
	КонецЕсли;
	
	ИмпортироватьДанныеPostman(ОбъектИмпорта.Данные);
КонецПроцедуры

&НаКлиенте
Асинх Функция ТекстФайлаИмпорта(ФайлИмпорта)
	Перем фРезультат;
	
#Если НЕ МобильноеПриложениеКлиент Тогда
	Чтение = Новый ТекстовыйДокумент;
	Ждать Чтение.ПрочитатьАсинх(ФайлИмпорта.ПолноеИмя, "UTF-8");
	
	фРезультат = Чтение.ПолучитьТекст();
#КонецЕсли
...
&НаКлиенте
Асинх Функция ОбъектФайлаИмпорта(ФайлИмпорта)
	фРезультат = Новый Структура("Данные, Отказ, ОписаниеОшибки", Неопределено, Ложь, "");
	
	ТекстJSON = Ждать ТекстФайлаИмпорта(ФайлИмпорта);
	Чтение = Новый ЧтениеJSON;
	Чтение.УстановитьСтроку(ТекстJSON);
	
	Попытка
		фРезультат.Данные = ПрочитатьJSON(Чтение, Истина);
		Чтение.Закрыть();
	Исключение
		фРезультат.Отказ = Истина;
		фРезультат.ОписаниеОшибки = ОписаниеОшибки();
	КонецПопытки;
...
Процедура ОбработатьОпциюCurl(ДанныеИмпорта, Знач ИдентификаторРесурсаИмпорт, Знач Опция, Отказ, ОписаниеОшибки)
	...
	ИначеЕсли ИмяОпции = "-O" ИЛИ ИмяОпции = "--remote-name" Тогда
		...
		ДанныеИмпорта.ИмяФайлаТелаОтвета = ОкончаниеИдентификатораРесурса;
...
Асинх Процедура ПрочитатьКомандуCurl(СтрокаКоманды)
	...
	ТелоОтветаВФайл = ЗначениеЗаполнено(ДанныеИмпорта.ИмяФайлаТелаОтвета);
	Если ТелоОтветаВФайл Тогда
		Каталог = Ждать КаталогВременныхФайловАсинх();
		ИмяФайлаТелаОтвета = Каталог + ДанныеИмпорта.ИмяФайлаТелаОтвета;
	КонецЕсли;
...
#Если НЕ МобильноеПриложениеСервер И НЕ МобильныйАвтономныйСервер Тогда
	фРезультат = ЗначениеВСтрокуВнутр(Данные);
#КонецЕсли
...
#Если НЕ МобильноеПриложениеСервер И НЕ МобильныйАвтономныйСервер Тогда
	фРезультат = ЗначениеИзСтрокиВнутр(СтрокаВнутр);
#КонецЕсли
...
Процедура ВыборФайлаИмпортаPostmanЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ПрочитатьФайлИмпорта(ВыбранныеФайлы[0]);
КонецПроцедуры
...
Процедура ВыборЭлементаИмпортаPostmanЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Отказ Тогда
		СообщитьПользователю(
			СтрШаблон("Не удалось импортировать запрос %1 по причине: %2", ДополнительныеПараметры[Результат]["name"], ОписаниеОшибки)
		);
	КонецЕсли;
КонецПроцедуры
...
#Если НЕ МобильноеПриложениеКлиент  Тогда
	...
	ТД.ЗаписатьАсинх(ВыбранныеФайлы[0]);
#КонецЕсли
КонецПроцедуры

&НаКлиенте
Асинх Процедура ВыборФайлаИмпортаИстории(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	Если ВыбранныеФайлы = Неопределено Тогда
		Возврат;
	КонецЕсли;
