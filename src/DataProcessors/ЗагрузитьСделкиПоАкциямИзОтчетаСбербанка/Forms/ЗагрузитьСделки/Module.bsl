...
Процедура ИмяФайлаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ru = 'Файлы Excel'") + "(*.xls, *.xlsx)|*.xls;*.xlsx";
...
Процедура СчитатьДанныеНаСервере(ДанныеФайла)
	...
	СчетаИзФайла = РезультатЧтения.Скопировать(,"НомерДоговора");	
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ПортфелиАкций.НомерДоговора ЕСТЬ NULL
		|			ТОГДА ВТ_СчетаИзФайла.НомерДоговора
		|		ИНАЧЕ ПортфелиАкций.Наименование
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПортфелиАкций КАК ПортфелиАкций
		|		ПО ВТ_СчетаИзФайла.НомерДоговора = ПортфелиАкций.НомерДоговора
		|		И НЕ ПортфелиАкций.ПометкаУдаления";
