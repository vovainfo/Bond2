...
Процедура ЗагрузитьС_MOEXНаСервере(Параметры, АдресРезультата) Экспорт
	...
	Для Каждого эл Из ТелоОтвета.changeover Цикл
		...
		Если эл.old_secid = эл.new_secid Тогда
			ПропущеноЗаписей = ПропущеноЗаписей + 1; 
			Продолжить;
		КонецЕсли;
	...
	КонецЦикла;
	СтрРезультат = СтрШаблон(НСтр("ru='Загружено %1 записей, пропущено %2 записей'"), 
																ЗагруженоЗаписей, ПропущеноЗаписей);
